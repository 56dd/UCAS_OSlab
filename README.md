...
最后开启O2编译后有bug，经发现需要修改crt0.S:

因为我在crt0.S中修改了用户栈的位置，但是并没有最后返回到内核栈，就会出现丢栈的问题，所以修改，使用t2寄存器来保存内核栈地址，最后返回到内核栈。

修改后O2编译成功。

### 任务5：批处理运行多个用户程序和管道输入

在这里我直接使用内存0x54000000作为管道（在下一个Pro中有待修缮）

于是在编写用户程序时，将所有的输出都存放到这个内存中，在后续程序中就可以直接打开。

这里输入batch就可以进入到批处理程序中。

这里批处理的思想也比较简单，定义一个batch函数，在这个函数内部分别加载每一个应用并运行，最后返回。
