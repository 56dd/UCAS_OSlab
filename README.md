...
代码位于sche.c的do_set_sche_workload中，我实现了两种调度算法，由于个人觉得两种算法最终实现效果都并不是完美，所以这里就不粘贴代码了，如果有兴趣就看文件中的代码吧，这里说一下思路：

方案1：计算速度，然后计算五个速度的平均数，对于大于平均速度的，减少一个时间片，大于平均速度的，增加一个时间片，保证至少有一个时间片

方案2：设置总时间片为60（可调节），然后根据五个飞机飞行的路程，算出平均数，算出每个与平均数的差，如果比平均数小记为正数，如果大于平均数记为0，然后按照这个数值比例分配时间片，设置至少分配一个时间片。

最终实现效果大致飞行速度相同。

#### 测试注意

测试时，对于A-core内容，请在main中110行for循环中，i=0到7，即测试前7个test，对于C-core内容，请在main中110行for循环中，i=7到12，即测试后5个test，另外删除sche中do_sche函数中的注释，开启时间片功能。如果想同时测试所有test，请将main中110行for循环中，i=0到12，删除sche中do_sche函数中的注释，开启时间片功能，最后修改fly1-fly5的FLY_OFF宏为12，这样防止不同程序屏幕重叠。
