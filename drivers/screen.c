...
void screen_write_ch(char ch)
{
    if (ch == '\n')
    {
        current_running[screen_cpu_id]->cursor_x = 0;
        if (current_running[screen_cpu_id]->cursor_y < SCREEN_HEIGHT)
            current_running[screen_cpu_id]->cursor_y++;
    }
    else if (ch == '\b' || ch == '\177')
    {	
        // TODO: [P3] support backspace here
        if(current_running[screen_cpu_id]->cursor_x !=0){
            current_running[screen_cpu_id]->cursor_x --;
            new_screen[SCREEN_LOC(current_running[screen_cpu_id]->cursor_x, current_running[screen_cpu_id]->cursor_y)] = ' ';
        }
        else{
            current_running[screen_cpu_id]->cursor_y = current_running[screen_cpu_id]->cursor_y - 1;
            current_running[screen_cpu_id]->cursor_x = SCREEN_WIDTH - 1;
            new_screen[SCREEN_LOC(current_running[screen_cpu_id]->cursor_x, current_running[screen_cpu_id]->cursor_y)] = ' ';
        }
    }
    ...
    {
        new_screen[SCREEN_LOC(current_running[screen_cpu_id]->cursor_x, current_running[screen_cpu_id]->cursor_y)] = ch;
        if (++current_running[screen_cpu_id]->cursor_x >= SCREEN_WIDTH)
        {
            current_running[screen_cpu_id]->cursor_x = 0;
            if (current_running[screen_cpu_id]->cursor_y < SCREEN_HEIGHT)
                current_running[screen_cpu_id]->cursor_y++;
...
void screen_clear(void)
{
    ...
    current_running[screen_cpu_id]->cursor_x = 0;
    current_running[screen_cpu_id]->cursor_y = 0;
    screen_reflush();
}
...
void screen_move_cursor(int x, int y)
{
    ...
    if (y >= SCREEN_HEIGHT)
        ...
        y = 0;
    current_running[screen_cpu_id]->cursor_x = x;
    current_running[screen_cpu_id]->cursor_y = y;
...
void screen_reflush(void)
{
    ...
    vt100_move_cursor(current_running[screen_cpu_id]->cursor_x + 1, current_running[screen_cpu_id]->cursor_y + 1);
}
